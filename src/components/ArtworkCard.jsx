...
export default function ArtworkCard({ artwork, onDelete, onQuickView }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 min-h-[600px] flex flex-col"
    ...
    >
      ...
      {/* Image Container */}
      <div className="relative h-[320px] rounded-t-2xl overflow-hidden bg-gray-100 flex-shrink-0">
        {imageError ? (
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-all duration-300">
          <div className="flex gap-2 justify-center">
            <motion.button
              ...
              className="flex-1 px-4 py-2 text-sm font-sans font-medium text-white bg-white/10 backdrop-blur-sm rounded-lg border border-white/20 hover:bg-white/20 transition-colors max-w-[140px]"
            >
              Quick View
            </motion.button>
            <Link
              to={`/artwork/${artwork.id}`}
              className="flex-1 px-4 py-2 text-sm font-sans font-medium text-white bg-indigo-500/80 backdrop-blur-sm rounded-lg hover:bg-indigo-600/80 transition-colors text-center max-w-[140px]"
            >
              View Details
            </Link>
          </div>
      ...
      <div className="p-6 flex-grow">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-5">
          <span className="px-3 py-1 text-sm font-sans font-medium bg-gray-100 text-gray-800 rounded-full">
            {artwork.style}
          </span>
          <span className="px-3 py-1 text-sm font-sans font-medium bg-gray-100 text-gray-800 rounded-full">
            {artwork.material}
