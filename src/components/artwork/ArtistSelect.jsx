...
/**
 * Searchable artist select dropdown with infinite-scroll.
 * @param {string} value - Currently selected artist ID.
 * @param {Function} onChange - Callback invoked with the selected artist ID.
 * @param {string} [className=""] - Additional CSS classes for the wrapper.
 * @param {boolean} [error=false] - Shows error border styling when true.
 */
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`relative w-full border rounded-xl shadow-sm py-3 px-4 pr-10 text-left bg-transparent font-sans text-base transition-colors outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:border-transparent md:text-sm ${
          isOpen
      ...
      >
        <span className={`block truncate ${selectedArtist ? "text-gray-900" : "text-muted-foreground"}`}>
          {selectedArtist
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-xl border border-gray-200 max-h-60 flex flex-col">
          <div className="p-2 border-b border-gray-100">
            <div className="relative group">
              <input
                ...
                className="w-full pl-9 pr-8 py-2 text-sm font-sans rounded-xl bg-transparent border border-gray-200 shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:border-transparent placeholder:text-gray-400 text-gray-900"
              />
