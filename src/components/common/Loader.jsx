 ...
 * @param {"xsmall"|"small"|"medium"|"large"} [size="medium"] - Size of the loader.
 * @param {string} [className=""] - Additional CSS classes.
 * @param {string} [color="indigo-600"] - Tailwind color token (e.g. "indigo-600", "white").
 */
const COLOR_CLASSES = {
  "indigo-600": {
    ring: "bg-gradient-to-r from-indigo-600 via-indigo-500 to-indigo-400",
    dot: "bg-indigo-600",
    deco: "bg-indigo-500",
  },
  "white": {
...
const Loader = ({ size = "medium", className = "", color = "indigo-600" }) => {
  const colors = COLOR_CLASSES[color] ?? COLOR_CLASSES["indigo-600"];
  ...
  return (
    <div className={`flex justify-center items-center ${className}`}>
      <div className={`relative ${containerSize[size]}`}>
        ...
        >
          <div
            className={`${sizeClasses[size]} scale-[0.2] rounded-full ${colors.dot}`}
          />
        </motion.div>
        ...
        {LOADER_POSITIONS.map((position, index) => (
          <motion.div
            key={position}
            className={`absolute w-2 h-2 rounded-full ${colors.deco}`}
            style={{
              [position]: "0",
              left:
